    ...
    <nav className="sticky top-0 z-50 border-b border-white/8 bg-black/88 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-5 px-4 py-2 sm:px-6 lg:px-8">
        <Link href="#home" className="flex items-center gap-3" onClick={() => setIsOpen(false)}>
          <Image src="/logo-pml-transparent.png" alt="PML logo" width={44} height={44} className="h-12 w-auto object-contain brightness-0 invert drop-shadow-[0_0_12px_rgba(212,175,55,0.35)]" />
          <div>
      ...
      <div className={`overflow-hidden border-t border-white/8 lg:hidden ${isOpen ? 'max-h-96' : 'max-h-0'} transition-all duration-300`}>
        <div className="space-y-1 px-4 py-2 sm:px-6">
          <a
            href={lawFirmConfig.pemenangKonsultanUrl}
            className="mt-2 inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-[#D4AF37]/40 bg-[#D4AF37]/10 px-4 py-2 text-sm font-bold text-[#F4D778]"
